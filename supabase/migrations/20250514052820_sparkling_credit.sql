/*
  # Add Portfolio System and Dynamic Valuation

  1. New Tables
    - `portfolios`
      - Tracks user portfolio values
      - Automatically created with new users
    
  2. Changes
    - Add market value tracking to artworks table
    - Add triggers for portfolio initialization
    - Add functions for dynamic valuation
...
-- Add market value columns to artworks
...
-- Create trigger to initialize portfolio
...
-- Create function to calculate market value
...
-- Create trigger to update market value on share transfer
...
BEGIN
  ...
  FOR user_id IN (
    ...
    PERFORM update_portfolio_value(user_id);
  END LOOP;
