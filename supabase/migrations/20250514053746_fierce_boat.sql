...
-- Create unique constraint for pending sales
CREATE UNIQUE INDEX physical_sales_pending_artwork_idx ON physical_sales (artwork_id) WHERE status = 'pending';
...
BEGIN
  ...
  FOR v_shareholder IN (
    ...
    UPDATE wallets
    SET balance = balance + v_shareholder.payout
