/*
  # Physical Sales System
  
  1. New Tables
    - physical_sales: Tracks physical artwork sale proposals
    - physical_sale_votes: Records shareholder votes on sale proposals
  
  2. Functions
    - propose_physical_sale: Creates new sale proposal
    - vote_on_physical_sale: Records shareholder votes
    - complete_physical_sale: Processes successful sales
    
  3. Security
    - RLS enabled on both tables
    - Policies for viewing sales and voting
*/
...
-- Create partial index for pending sales if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes 
    WHERE indexname = 'physical_sales_pending_artwork_idx'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'physical_sales' 
    AND policyname = 'Anyone can view physical sales'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'physical_sale_votes' 
    AND policyname = 'Users can vote on physical sales'
  ) THEN
